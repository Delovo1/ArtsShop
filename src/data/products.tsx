export const artProducts = [
  {
    id: 1,
    title: "Звездная ночь",
    artist: "Винсент Ван Гог",
    price: 18900,
    size: "73x92 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ea/Van_Gogh_-_Starry_Night_-_Google_Art_Project.jpg/800px-Van_Gogh_-_Starry_Night_-_Google_Art_Project.jpg",
    description: "Одна из самых узнаваемых картин в мире. Написана в 1889 году в лечебнице Сен-Реми. Вихревое небо передает эмоциональное состояние художника.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 2,
    title: "Крик",
    artist: "Эдвард Мунк",
    price: 15700,
    size: "91x73 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f4/The_Scream.jpg/800px-The_Scream.jpg",
    description: "Символ экзистенциального страха. Мунк создал четыре версии этой картины, передающих чувство отчаяния и тревоги современного человека.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 3,
    title: "Девушка с жемчужной сережкой",
    artist: "Ян Вермеер",
    price: 24500,
    size: "44x39 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/0f/1665_Girl_with_a_Pearl_Earring.jpg/800px-1665_Girl_with_a_Pearl_Earring.jpg",
    description: "«Северная Мона Лиза». Вермеер мастерски передал свет и тень, создав интимный портрет девушки, повернувшей голову к зрителю.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 4,
    title: "Рождение Венеры",
    artist: "Сандро Боттичелли",
    price: 32400,
    size: "172x278 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/0b/Sandro_Botticelli_-_La_nascita_di_Venere_-_Google_Art_Project_-_edited.jpg/800px-Sandro_Botticelli_-_La_nascita_di_Venere_-_Google_Art_Project_-_edited.jpg",
    description: "Шедевр эпохи Возрождения. Богиня любви плывет к берегу в раковине, символизируя красоту и гармонию античного мира.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 5,
    title: "Мона Лиза",
    artist: "Леонардо да Винчи",
    price: 999900,
    size: "77x53 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ec/Mona_Lisa%2C_by_Leonardo_da_Vinci%2C_from_C2RMF_retouched.jpg/800px-Mona_Lisa%2C_by_Leonardo_da_Vinci%2C_from_C2RMF_retouched.jpg",
    description: "Самая загадочная улыбка в истории. Леонардо использовал технику сфумато, создавая неуловимую дымку и объем.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 6,
    title: "Поцелуй",
    artist: "Густав Климт",
    price: 28500,
    size: "180x180 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/4/40/The_Kiss_-_Gustav_Klimt_-_Google_Cultural_Institute.jpg/800px-The_Kiss_-_Gustav_Klimt_-_Google_Cultural_Institute.jpg",
    description: "Золотой период Климта. Влюбленные застыли в вечном объятии, окруженные орнаментом и сиянием сусального золота.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 7,
    title: "Американская готика",
    artist: "Грант Вуд",
    price: 9800,
    size: "78x65 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/7/71/Grant_DeVolson_Wood_-_American_Gothic.jpg/1280px-Grant_DeVolson_Wood_-_American_Gothic.jpg",
    description: "Икона американского искусства. Суровые фермеры с вилами символизируют дух Среднего Запада времен Великой депрессии.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 8,
    title: "Ночное кафе",
    artist: "Винсент Ван Гог",
    price: 18700,
    size: "72x92 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/5/5e/Vincent_Willem_van_Gogh_076.jpg",
    description: "Ван Гог писал: «Я пытался выразить красным и зеленым ужасные страсти человеческие». Напряженные цвета передают атмосферу ночного заведения.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 9,
    title: "Автопортрет",
    artist: "Винсент Ван Гог",
    price: 21500,
    size: "65x54 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b2/Vincent_van_Gogh_-_Self-Portrait_-_Google_Art_Project.jpg/800px-Vincent_van_Gogh_-_Self-Portrait_-_Google_Art_Project.jpg",
    description: "Один из 40 автопортретов Ван Гога. Энергичные мазки и напряженный взгляд отражают внутреннюю борьбу художника.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 10,
    title: "Ирисы",
    artist: "Винсент Ван Гог",
    price: 16800,
    size: "71x93 см",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQhh6UKpJoRrsv8DW-GoJikF9NTXBm-HROEzA&s",
    description: "Написана в больнице Сен-Поль. Яркие ирисы на желтом фоне символизируют надежду и жизнь вопреки обстоятельствам.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 11,
    title: "Сад земных наслаждений",
    artist: "Иероним Босх",
    price: 45700,
    size: "220x389 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/a/ae/El_jard%C3%ADn_de_las_Delicias%2C_de_El_Bosco.jpg",
    description: "Триптих-загадка. Рай, земные наслаждения и ад переплетены в причудливых образах, полных символизма и фантазии.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 12,
    title: "Свобода, ведущая народ",
    artist: "Эжен Делакруа",
    price: 32800,
    size: "260x325 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/02/La_Libert%C3%A9_guidant_le_peuple_-_Eug%C3%A8ne_Delacroix_-_Mus%C3%A9e_du_Louvre_Peintures_RF_129_-_apr%C3%A8s_restauration_2024.jpg/1280px-La_Libert%C3%A9_guidant_le_peuple_-_Eug%C3%A8ne_Delacroix_-_Mus%C3%A9e_du_Louvre_Peintures_RF_129_-_apr%C3%A8s_restauration_2024.jpg",
    description: "Символ Французской революции. Марианна с флагом ведет народ через баррикады, объединяя разные слои общества.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 13,
    title: "Охотники на снегу",
    artist: "Питер Брейгель Старший",
    price: 38900,
    size: "117x162 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ed/Pieter_Bruegel_d._%C3%84._106b.jpg/500px-Pieter_Bruegel_d._%C3%84._106b.jpg",
    description: "Зимний пейзаж, где охотники возвращаются с добычей. Застывшая природа и быт крестьан создают атмосферу спокойствия.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 14,
    title: "Тайная вечеря",
    artist: "Леонардо да Винчи",
    price: 785000,
    size: "460x880 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4b/%C3%9Altima_Cena_-_Da_Vinci_5.jpg/800px-%C3%9Altima_Cena_-_Da_Vinci_5.jpg",
    description: "Момент, когда Христос объявляет о предательстве. Леонардо передал эмоции каждого апостола в драматической сцене.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 15,
    title: "Водяные лилии",
    artist: "Клод Моне",
    price: 42800,
    size: "200x426 см",
    image: "https://muzei-mira.com/templates/museum/images/paint/vodianie-lilii-mone+.jpg",
    description: "Серия из 250 картин с кувшинками в саду Живерни. Моне исследовал изменения света и отражений на воде.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 16,
    title: "Завтрак гребцов",
    artist: "Пьер Огюст Ренуар",
    price: 36700,
    size: "130x173 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8d/Pierre-Auguste_Renoir_-_Luncheon_of_the_Boating_Party_-_Google_Art_Project.jpg/1280px-Pierre-Auguste_Renoir_-_Luncheon_of_the_Boating_Party_-_Google_Art_Project.jpg",
    description: "Солнечный день на террасе. Ренуар передал радость жизни и дружеского общения через игру света и тени.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 17,
    title: "Бал в Мулен де ла Галетт",
    artist: "Пьер Огюст Ренуар",
    price: 41900,
    size: "131x175 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/2/21/Pierre-Auguste_Renoir%2C_Le_Moulin_de_la_Galette.jpg",
    description: "Танцы в воскресный день на Монмартре. Ренуар запечатлел атмосферу парижского развлечения и флирта.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 18,
    title: "Купание красного коня",
    artist: "Кузьма Петров-Водкин",
    price: 29500,
    size: "160x186 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/a/af/Bathing_the_Red_Horse_P1060789.JPG",
    description: "Символ новой России. Красный конь на фоне голубой воды вызывает ассоциации с судьбой страны и народа.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 19,
    title: "Девятый вал",
    artist: "Иван Айвазовский",
    price: 54800,
    size: "221x332 см",
    image: "https://xn--90add8ag.xn--p1ai/wp-content/uploads/2024/09/GLAVNAYA-7.jpg",
    description: "Борьба людей со стихией после шторма. Люди цепляются за обломки мачты, а гигантская волна готова обрушиться.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 20,
    title: "Утро в сосновом лесу",
    artist: "Иван Шишкин",
    price: 38700,
    size: "139x213 см",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRA4FG8tG4G8wmk-PNW1UblsnOzzUMj9H8XYQ&s",
    description: "Медвежата играют на сломанном дереве. Лесная глушь просыпается, наполняясь утренним светом и жизнью.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 21,
    title: "Боярыня Морозова",
    artist: "Василий Суриков",
    price: 42900,
    size: "304x587 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/2/2b/Boyaryna_Morozova_by_V.Surikov_%281884-1887%2C_Tretyakov_gallery%29_detail_01.jpg",
    description: "Раскольница Морозова отказывается от новых обрядов. Трагический образ русской истории, сила веры и духа.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 22,
    title: "Не ждали",
    artist: "Илья Репин",
    price: 36500,
    size: "160.5x167.5 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/1/16/Ilya_Repin_Unexpected_visitors.jpg",
    description: "Возвращение ссыльного домой. Семья замерла в момент узнавания — от недоверия до радости.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 23,
    title: "Грачи прилетели",
    artist: "Алексей Саврасов",
    price: 29800,
    size: "62x48.5 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/1/12/RooksBackOfSavrasov.jpg",
    description: "Весна в русской глубинке. Тающий снег, березы и прилетевшие грачи создают лирическое настроение.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 24,
    title: "Черный супрематический квадрат",
    artist: "Казимир Малевич",
    price: 12500,
    size: "79.5x79.5 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/%D0%A7%D1%91%D1%80%D0%BD%D1%8B%D0%B9_%D1%81%D1%83%D0%BF%D1%80%D0%B5%D0%BC%D0%B0%D1%82%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9_%D0%BA%D0%B2%D0%B0%D0%B4%D1%80%D0%B0%D1%82._1915._%D0%93%D0%A2%D0%93.png/500px-%D0%A7%D1%91%D1%80%D0%BD%D1%8B%D0%B9_%D1%81%D1%83%D0%BF%D1%80%D0%B5%D0%BC%D0%B0%D1%82%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9_%D0%BA%D0%B2%D0%B0%D0%B4%D1%80%D0%B0%D1%82._1915._%D0%93%D0%A2%D0%93.png",
    description: "Манифест нового искусства. Малевич объявил квадрат «нулем форм», освобождая живопись от предметности.",
    inBasket: false,
    quantity: 0,
    entered: true,
  },
  {
    id: 25,
    title: "Композиция VIII",
    artist: "Василий Кандинский",
    price: 67800,
    size: "140x201 см",
    image: "https://upload.wikimedia.org/wikipedia/commons/4/47/Vassily_Kandinsky%2C_1923_-_Composition_8%2C_huile_sur_toile%2C_140_cm_x_201_cm%2C_Mus%C3%A9e_Guggenheim%2C_New_York.jpg",
    description: "Абстрактная симфония цвета и форм. Круги, линии и геометрические фигуры создают ритм, подобный музыке.",
    inBasket: false,
    quantity: 0,
    entered: true,
  }
];